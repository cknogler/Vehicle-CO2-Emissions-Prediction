...
def deduplicate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Only dedup on columns that actually exist and have no all-NaN
    keys = [k for k in DEDUP_KEYS if k in df_f.columns and df_f[k].notna().any()]
    if not keys:
        return df_f
...
def train_models(df_bytes: bytes):
    ...
    # Alle Spalten robust zu numerisch konvertieren und median-imputen
    for col in avail:
        if col in df_m.columns:
            try:
                df_m[col] = pd.to_numeric(df_m[col], errors="coerce")
                if df_m[col].dtype in [np.float64, np.int64, float, int]:
                    df_m[col] = df_m[col].fillna(df_m[col].median())
            except Exception:
                pass
 
    # Kategoriale Spalten encodieren
    ...
    # Nochmals alle NaN entfernen (Sicherheitsnetz)
    df_m = df_m.dropna(subset=avail + [TARGET])
 
    X = df_m[avail].astype(float).values
    y = df_m[TARGET].astype(float).values
 
    # Einheitlicher split fuer alle Modelle
