...
with tabs[5]:
    ...
    # ── Isolierter Getriebe-Effekt (Ceteris Paribus) ─────────────────────────
    st.markdown("---")
    st.subheader("Isolierter Getriebe-Effekt auf CO2 (Ceteris Paribus)")
    st.markdown(
        "Alle anderen Fahrzeugmerkmale werden auf den **Datensatz-Median** fixiert "
        "(Masse, Leistung, Kraftstoff, Karosserie). Nur das Getriebe variiert. "
        "So zeigt das Modell den **reinen Getriebe-Effekt** auf CO2 - "
        "bereinigt von Konfundierungsvariablen wie Fahrzeugtyp oder Leistung."
    )
    try:
        base = {}
        for f in feature_cols:
            if f in num_f:
                base[f] = float(df_unique[f].median())
            else:
        ...
        bar_cols = ["#2ecc71" if v < base_pred - 1 else "#e74c3c" if v > base_pred + 1
                    else BLUE for v in gear_df["CO2_pred"]]
        ...
        axes[0].set_xlabel("Vorhergesagter CO2 (g/km)")
        axes[0].set_title("Absoluter CO2 je Getriebetyp (alle anderen Features = Median/Modus)", fontsize=11)
        axes[0].legend(fontsize=9)
        for sp in ["top", "right"]:
            axes[0].spines[sp].set_visible(False)

        delta_cols = ["#e74c3c" if d > 1 else "#2ecc71" if d < -1 else BLUE
                      for d in gear_df["Delta"]]
        bars2 = axes[1].barh(gear_df["Label"], gear_df["Delta"],
                             color=delta_cols, alpha=0.85, edgecolor="white")
        axes[1].axvline(0, color="black", lw=1, linestyle="-")
        for bar, val in zip(bars2, gear_df["Delta"]):
            xpos = val + 0.3 if val >= 0 else val - 0.3
            ha = "left" if val >= 0 else "right"
            axes[1].text(xpos, bar.get_y() + bar.get_height() / 2,
                         ("+" if val >= 0 else "") + str(val), va="center", ha=ha, fontsize=9)
        axes[1].set_xlabel("CO2-Differenz vs. Ø Schaltgetriebe (g/km)")
        axes[1].set_title("Getriebe-Effekt relativ zu Ø Schaltgetriebe (ceteris paribus)", fontsize=11)
        for sp in ["top", "right"]:
            axes[1].spines[sp].set_visible(False)

        b_mass = str(round(base.get("Empty Mass Euro Avg (kg)", 0), 0))
        b_kw   = str(round(base.get("Maximum Power (kW)", 0), 0))
        b_fuel = str(base.get("Fuel", "-"))
        b_body = str(base.get("Body", "-"))
        fig.suptitle("Isolierter Getriebe-Effekt | Basis: " + b_mass + " kg, " + b_kw + " kW, Fuel=" + b_fuel + ", Body=" + b_body, fontsize=11, y=1.01)
        plt.tight_layout()
        st.pyplot(fig)
        plt.close()
        ...
        if len(auto_vals) > 0 and len(manual_vals) > 0:
            ...
            if abs(diff) < 2:
                ...
                st.warning("Automatik hat " + str(diff) + " g/km mehr CO2 als Schaltgetriebe bei sonst identischer Konfiguration (ceteris paribus).")
            else:
                st.success("Automatik hat " + str(abs(diff)) + " g/km weniger CO2 als Schaltgetriebe bei sonst identischer Konfiguration (ceteris paribus).")

    except Exception as e:
        st.warning("Getriebe-Analyse nicht verfuegbar: " + str(e))
